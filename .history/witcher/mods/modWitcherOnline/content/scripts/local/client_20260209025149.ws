                                        ...
                                        cpcItem1 : string, cpcItem2 : string, cpcItem3 : string, cpcItem4 : string, cpcItem5 : string, cpcItem6 : string, cpcItem7 : string, cpcItem8 : string, cpcItem9 : string, cpcItem10 : string) 
    {
        ...
        for (i = 0; i < globalPlayers.Size(); i += 1)
        {
            if (globalPlayers[i].id == id)
            {
                ...
                // Detect Party Change for Notifications
                if(partyName != globalPlayers[i].partyName)
                {
                    myParty = theGame.r_getMultiplayerClient().partyManager.GetPartyName();
                    if(StrLen(myParty) > 0)
                    {
                        if(globalPlayers[i].partyName == myParty && partyName != myParty)
                        {
                            theGame.GetGuiManager().ShowNotification(cleanUsername + " left your party.");
                        }
                        else if(partyName == myParty && globalPlayers[i].partyName != myParty)
                        {
                            theGame.GetGuiManager().ShowNotification(cleanUsername + " joined your party.");
                        }
                    }
                }
